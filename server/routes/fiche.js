  ...
  try {
    ...
    try {
      for (const item of items) {
        const doc = await db.createDocument(DATABASE_ID, COLLECTION_FICHE_ID, ID.unique(), item);
        created.push(doc.$id);
      }
    } catch (err) {
      for (const id of [...created].reverse()) {
        try { await db.deleteDocument(DATABASE_ID, COLLECTION_FICHE_ID, id); } catch {}
      }
      return res.status(500).json({ error: err.message });
    }

    res.json({ message: "Fiche created successfully", fiche: created });
  } catch (error) {
